...
export const PopupRequestDemoAssets = () => {
    ...
    return (
        <>
            {connected && isLoaded && demoNFTsCount === 0 && demoTokensBalance === 0 && (
                <div className="fixed flex flex-col w-full gap-2.5 p-4 -translate-x-1/2 -translate-y-1/2 border max-w-max bg-slate-800 border-slate-600 rounded-2xl top-1/2 left-1/2">
                    ...
                    <button onClick={getDemoAssets} type="button" className="font-semibold duration-200 sm:text-lg h-9 rounded-xl bg-sky-500 hover:bg-sky-400 hover:scale-95 active:scale-100">Request Demo Assets</button>
                </div>
            )}
        </>
    )
